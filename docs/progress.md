...
## 2026-09-21：协作文档提交与公开快照语义

### 已完成

- 已发布文档根据规范化 authoring hash 判断“已是最新/更新”，不再仅按 published 状态启用更新。
- IndexedDB 继续即时保存；Yjs WebSocket 增量按 300ms 空闲、1200ms 最大等待合并，并用有序同步屏障确认持久化。
- Studio 增加显式保存、离开/发布提交、历史时间线与恢复入口；恢复不会覆盖既有提交。
- 标题附近增加隐式 Emoji、摘要、标签和封面入口；公开文章、首页精选卡片和编辑器复用已发布封面。
- 公开查询优先读取有效快照中的 hash、图标和封面字段，并继续排除回收站和取消发布文档。

### 当前边界

- 提交目前以完整富文本锚点保存，后续可在 Collaboration 分支接入压缩增量和 Agent 修订操作；恢复请求已具备幂等接口，跨服务 CRDT 重置仍需部署协同 RPC。

## 2026-09-11：文档编辑器 slash 与选区浮动工具栏

Studio 文档画布在段落开头输入 `/` 可打开 slash 菜单（标题、列表、任务、引用、代码、分隔线、表格），方向键/Enter/Escape 可键盘操作。选中文本显示浮动格式栏（粗体/斜体/下划线/删除线/链接/对齐）。顶栏只保留撤销/重做与发布，避免常驻格式按钮。
