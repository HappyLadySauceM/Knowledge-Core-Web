...
Knowledge Core 的桌面优先、响应式 Web 前端。当前实现包含 Next.js App Router UI、设计系统、通过 HttpOnly cookie 管理会话的同源 BFF，以及文档、页面树、成员、实时协作、媒体库和管理配置界面。文档保留实时编辑稿、最近一次公开快照和自动历史时间线：编辑稿持续协作同步，公开页只在发布/更新时替换快照；Studio 不提供重复的手动保存按钮，历史恢复会写回实时草稿并等待 Collaboration 确认。
...
工作流通过固定 SHA 的 `ci-templates` 动作恢复带摘要校验的本地 Artifact 缓存，远端下载最多五次指数退避，跨节点 Artifact 上传完整重试一次且连续失败仍阻断发布；发布前会从候选制品恢复同一 digest 的 Harbor 标签，防止 failed-only rerun 因标签被误删而失败。每日 `maintenance.yml` 使用独立 `maintenance` Environment，以 fail-closed 方式清理超过 72 小时的候选和运行制品，并在显式缓存目录超过 80% 使用率时按 LRU 回收到 70%。
...
- Studio 通过领域客户端接入 Gateway，支持“我的文档库 / 知识空间”树形导航、回收站、永久删除、发布/更新、成员权限、自动历史时间线和 Yjs 实时协作；文档从列表行移入回收站，编辑页不再暴露文档设置删除入口。编辑稿由 IndexedDB 即时落盘，WebSocket 增量在 300ms 空闲或 1200ms 最大等待后合并提交，公开页只读取最近一次发布快照并保留快照中的空段落。历史恢复先把快照应用到 Yjs 草稿并等待服务端确认，再恢复展示元数据；公开状态保持不变，恢复后必须点击“更新”才会替换公开快照。已发布文档根据忽略历史块 ID 的规范化作者内容 hash 计算“已是最新/更新”，不会因为连接握手、系统归一化或普通重渲染误启用更新。标题附近可隐式添加 Emoji 图标、摘要、标签和封面，封面同时用于编辑器、公开文章和首页卡片。文档、媒体、管理配置和账号安全使用独立的后台 Shell，公开站点导航不会复用到后台。
