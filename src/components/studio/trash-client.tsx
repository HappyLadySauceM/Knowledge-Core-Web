...
export function TrashClient({ locale }: { locale: string }) {
  ...
  return <section className="management-page"><label className="management-search"><Search size={15} /><input value={q} onChange={(event) => setQ(event.target.value)} placeholder={locale === "zh-CN" ? "搜索已删除文档" : "Search deleted documents"} /></label>{trash.error && <p className="form-error">{trash.error.message}</p>}<div className="management-list">{trash.data?.items.map((document) => <article key={document.id}><div><strong>{document.title}</strong><span>{new Date(document.deleted_at ?? document.updated_at).toLocaleString(locale)}</span></div><Button variant="outline" size="sm" onClick={() => restore.mutate(document.id)} disabled={restore.isPending}><RotateCcw size={14} />{locale === "zh-CN" ? "恢复" : "Restore"}</Button></article>)}</div>{!trash.isLoading && !trash.data?.items.length && <p className="empty-inline">{locale === "zh-CN" ? "回收站是空的。" : "Trash is empty."}</p>}</section>;
}
