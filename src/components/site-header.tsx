...
export function SiteHeader({ locale, profile }: { locale: string; profile?: SiteProfile }) {
  ...
  return <header className="site-header"><div className="site-header-inner container-shell"><Link href={`/${locale}`} className="brand"><span className="brand-mark">HS</span><span>{profile?.title ?? "HappyLadySauce"}</span></Link><nav className="site-nav"><Link href={`/${locale}`}>{t.nav.explore}</Link><Link href={`/${locale}/studio`}>{t.nav.studio}</Link>{user && <Link href={`/${locale}/studio/media`}>{locale === "zh-CN" ? "媒体" : "Media"}</Link>}{user?.role === "admin" && <Link href={`/${locale}/admin`}>{locale === "zh-CN" ? "管理" : "Admin"}</Link>}</nav><div className="header-actions"><LanguageSwitcher locale={locale} /><ThemeToggle />{user ? <div className="account-actions"><Link href={`/${locale}/settings/security`}>{user.username}</Link><button type="button" onClick={() => void logout()}>{locale === "zh-CN" ? "退出" : "Sign out"}</button></div> : <Button asChild size="sm"><Link href={`/${locale}/login`}>{t.nav.signIn}</Link></Button>}</div></div></header>;
}
