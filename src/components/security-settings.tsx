...
async function authFetch(path: string, init?: RequestInit) {
  const response = await fetch(`/api/bff/auth/${path}`, init);
...
export function SecuritySettings({ locale = "zh-CN" }: { locale?: string }) {
  ...
  async function deactivate(event: FormEvent<HTMLFormElement>) {
    ...
    if (!confirm("Deactivate your account? This signs out every device.")) return;
    const password = new FormData(event.currentTarget).get("password");
    try {
      await authFetch("deactivate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ password }) });
      router.push(`/${locale}`);
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to deactivate account");
    }
  }
  const error = sessions.error || revoke.error || revokeAll.error || account.error;
    ...
    <div className="settings-shell container-shell">
      ...
      {verified ? (
        <p className="form-success" role="status">
          {locale === "zh-CN" ? "邮箱已验证" : "Email verified"}
        </p>
      ) : null}
        ...
        {sessions.data?.items.map((session) => (
          <article className="session-row" key={session.id}>
            <div>
              ...
              <span>{session.current ? "This device" : "Other device"} · Last active {new Date(session.last_seen_at).toLocaleString()}</span>
            </div>
            {!session.current && (
      ...
      <Button variant="secondary" onClick={() => revokeAll.mutate()}>
        Sign out all other devices
      </Button>
      <section className="danger-zone">
        <h2>Deactivate account</h2>
        <p>Your account and all active sessions will be disabled.</p>
