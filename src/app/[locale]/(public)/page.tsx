...
export default async function Home({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  return <div className="editorial-home">
    ...
    <section id="articles" className="articles-shell container-shell">
      ...
      {featured ? <Link href={`/${locale}/articles/${featured.slug}`} className="featured-article"><div><p className="article-kicker">{featured.tags?.[0] ?? t.home.featured}</p><h3>{featured.icon ? `${featured.icon} ` : ""}{featured.title}</h3><p>{featured.summary || t.home.readMore}</p><span className="article-meta">{featured.owner?.username} · {formatDate(featured.published_at ?? featured.updated_at, locale)} <ArrowUpRight size={16} /></span></div><div className="featured-art" aria-hidden="true" style={featured.cover_attachment_id ? { backgroundImage: `url(/api/bff/gateway/api/v1/attachments/${encodeURIComponent(featured.cover_attachment_id)}/content)`, backgroundPosition: `${featured.cover_focal_x ?? 50}% ${featured.cover_focal_y ?? 50}%` } : undefined} /></Link> : null}
      {rest.length > 0 ? <div className="article-grid">{rest.map((document) => <Link key={document.id} href={`/${locale}/articles/${document.slug}`} className="article-card"><p className="article-kicker">{document.tags?.[0] ?? t.home.article}</p><h3>{document.icon ? `${document.icon} ` : ""}{document.title}</h3><p>{document.summary || t.home.readMore}</p><span className="article-meta">{formatDate(document.published_at ?? document.updated_at, locale)} <ArrowUpRight size={15} /></span></Link>)}</div> : null}
