...
export type SessionData = { user: SessionUser; expires_at?: string };
...
export type MediaAttachment = { id: string; owner_id: number; filename: string; media_type: string; category: string; size_bytes: number; sha256: string; status: string; part_size: number; part_count: number; created_at: string; detected_type?: string; content_url: string };
export type MediaAttachmentPart = { part_number: number; url: string; expires_at: string };
export type MediaAttachmentUpload = { attachment: MediaAttachment; upload_id: string; parts: MediaAttachmentPart[]; expires_at: string };
export type CollaborationSession = { websocket_url: string; ticket: string; subprotocol: string; fragment: string; access: string; ticket_expires_at: string; session_expires_at: string };
